...
exports.getStockReport = async (req, res) => {
	try {
		...
		const formattedResponse = Object.keys(groupedStockHistory).map(
			(recordDate) => ({
				recordDate,
	...
	try {
		...
		if (format === "csv") {
			...
			const csv = json2csvParser.parse(
				stockHistory.map((record) => record.dataValues)
			);
			...
			res
				.status(400)
				.json({ message: 'Unsupported format. Please use "excel" or "csv".' });
		}
	} catch (error) {
